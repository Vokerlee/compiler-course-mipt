...
//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2021-07-02 21:41:48 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-07-02 21:41:48 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-07-02 21:41:48 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-07-02 21:41:48 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!
//!              bool oldPSW = _txProcessSysErrors;
//!              _txProcessSysErrors = false;            // Just do it. (C) NAN - Not a Nike :)
//!
//!              if ($( ComDlg32::GetOpenFileNameA ))
//!                  $( ComDlg32::GetOpenFileNameA (&ofn) );  // Весьма полезная функция, отображает диалог выбора файла.
//!
//!              _txProcessSysErrors = oldPSW;           // Just do it too.
//!
...
//}----------------------------------------------------------------------------------------------------------------
       ...
       bool _txProcessSysErrors           = true;

#endif // TX_COMPILED

extern bool _txProcessSysErrors;
...
long WINAPI _txVectoredExceptionHandler (EXCEPTION_POINTERS* exc)
    ...
    if (!_txProcessSysErrors)
        ...
        if (code != DBG_PRINTEXCEPTION_C &&
            ...
            {
            txOutputDebugPrintf ("%s - WARNING: %s (code 0x%08lX, addr 0x%p) called and SKIPPED! (_txProcessSysErrors == %d)\n",
                                 _TX_VERSION, "_txVectoredExceptionHandler", (unsigned long) code, addr, _txProcessSysErrors);
            }
...
$   Win32::ITaskbarList3* taskbar = NULL;
...
$   ok &= !!taskbar && (res == S_OK);
    ...
    if (ok && taskbar)     {$ res = taskbar->SetProgressState (wnd, type);                 ok &= (res == S_OK); }
