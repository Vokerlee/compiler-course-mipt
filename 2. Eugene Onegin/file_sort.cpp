...
char *read_file (FILE *onegin, size_t *length_file, size_t *lines_file)
{
    ...
    *length_file = length_of_file(onegin);
    ...
    char *stream = (char *) calloc(*length_file, sizeof(char));
    assert(stream);
...
void sortlines (FILE *onegin, FILE *res, const char *regime, const char *bred_ind)
{
    ...
    line_t *lines = (line_t*) calloc(file_lines, sizeof(line_t));
    assert(lines);

    write_in_structs(lines, stream, &n_symb, &n_struct, &file_length);

    int (*str_comp)(const void*, const void *) = choose_comp(regime);
