...
void div_optimize (bin_tree_elem *element, int *optimize_state)
{
    ...
    if ((int) element->value == DIV && L->type == VAR && R->type == VAR && (int) L->value == (int) R->value)
    {
        element->type  = NUM;
        element->value = 1;
...
void undertree_optimize (bin_tree_elem *element)
{
    int optimize_state = 0;
    int prev_state     = -1;

    diff_optimize(element);

    while(optimize_state != prev_state)
    {
        prev_state = optimize_state;

        const_fold_optimize(element, &optimize_state);

        num_optimize(element, &optimize_state);
    }
    ...
    while(optimize_state != prev_state)
    {
        ...
        div_optimize(element, &optimize_state);
    }
}
